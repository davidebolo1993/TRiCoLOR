...
int main(int argc, char* argv[]) {
	...
	boost::filesystem::path fmill(container.fmill); // is a directory containin illumina .fmi indexes
	...
	while(bcf_read(bcf, header, record) == 0) {
		...
		if ((rec.gen1 == 1) && ((rec.gen2 == -1) || (rec.gen2 == 0))) {
			...
			else if (seq1.size() > 124) { // this could not be confirmed as sequences are shorter
			...
			else if (seq2.size() > 124) { // this could not be confirmed as sequences are shorter
	...
	myfile << "Variants too long for illumina sequences: " << toolong << std::endl;	
	myfile.close();
