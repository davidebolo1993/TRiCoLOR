...
int main(int argc, char **argv) {
    ...
    boost::filesystem::path fastq(argv[1]);
    boost::filesystem::path modfastq(argv[2]);
    boost::filesystem::path fm(argv[3]);


    boost::posix_time::ptime timer;

    std::ifstream input(fastq.string(), std::ios_base::in | std::ios_base::binary);
    ...
    // first, check if the input is gzipped
        ...
        if (((uint8_t)fcode[0] == (uint8_t)0x1f) && ((uint8_t)fcode[1] == (uint8_t)0x8b)) { // check for magic numbers (gzipped input??)
            ...
            std::ifstream to_stream(fastq.string(), std::ios_base::in | std::ios_base::binary);
            ...
            //Iterate over lines
            std::string line;
            std::ofstream output(modfastq.string());
            ...
            while (std::getline(instream, line)) {
                ...
                else if (toupper(line[0]) == 'A' || toupper(line[0]) == 'C' || toupper(line[0]) == 'T' || toupper(line[0]) == 'G' ||  line[0] == 'N'){ // this is the sequence, works with both .fasta and .fastq
        ...
        construct(fm_index,modfastq.string(), 1); // construct from file, avoid problems with memory ?
